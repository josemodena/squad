...
# issue. Every project-specific value comes from .codex/squad.local.md.
set -euo pipefail
...
case "${1:-}" in
  ready|recover|metrics|wake|state|models|policy|pause|resume|observe|field|dependency|claim|bind|checkpoint|complete|ack|retry|external|settle|comment|issue-read|pr-read|issue-create|run|review-prepare|review-record)
...
The tracks, the owners and the statuses come from .codex/squad.local.md.
...
field_id() {
  ...
  [ -n "$id" ] || squad_die "The project has no field named '$name'. Run the Squad init skill to create the fields."
  printf '%s\n' "$id"
}
...
cmd_quota_note() {
  ...
  if [ -z "$quota_bin" ] && [ -x "$SCRIPT_DIR/$SQUAD_QUOTA_COMMAND" ]; then quota_bin="$SCRIPT_DIR/$SQUAD_QUOTA_COMMAND"; fi
  [ -n "$quota_bin" ] || squad_die "The quota command '$SQUAD_QUOTA_COMMAND' is not on PATH."

  local stamp quota_out body
  stamp="$(date -u '+%Y-%m-%d %H:%M UTC')"
  quota_out="$("$quota_bin" --json 2>&1 || true)"

  body="$(printf '**Quota reading, %s**\n\n`%s --json`\n\n```json\n%s\n```\n' \
    "$stamp" "$SQUAD_QUOTA_COMMAND" "$quota_out")"
