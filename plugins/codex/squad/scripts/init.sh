...
Usage:
  ...
  init.sh create-board "<title>"
      ...
      .codex/squad.local.md as project_number, then run apply.
...
label_description() {
  case "$1" in
    ...
    review:passed)   printf 'Independent review passed; finish may merge' ;;
...
wanted_labels() {
  ...
  printf '%s\n' 'type:piece' 'type:sprint' 'type:retro' 'type:bug' 'type:enhancement' 'type:documentation' \
    "$SQUAD_DECIDER_LABEL" 'decision-needed' 'blocked' 'review:passed'
...
starts and what the project may spend are all in \`.codex/squad.local.md\`.
...
- **The board is the state.** Every piece of work is an issue whose body is its
  plan. Start with the Squad quota and board skills. Move cards as pieces move.
...
cmd_apply() {
  ...
  squad_say "Done"
  printf 'The repository is set up. Use the Squad board skill to see the empty board.\n'
}
