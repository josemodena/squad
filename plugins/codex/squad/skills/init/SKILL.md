...
`.codex/squad.local.md`, holds the repository, the board, the people, the
...
```bash
bash ${PLUGIN_ROOT}/scripts/init.sh check
```
...
Ask the user in one concise pass and do not invent project-specific answers.
...
2. **The board.** Does a project board exist? If yes, its number. If no, offer
   to create one: `bash ${PLUGIN_ROOT}/scripts/init.sh create-board "<title>"`
...
Copy `${PLUGIN_ROOT}/skills/init/assets/squad.local.md` to
`.codex/squad.local.md` in the project and replace every value with the
...
```bash
bash ${PLUGIN_ROOT}/scripts/squad.sh settings
```

### 4. Create everything

```bash
bash ${PLUGIN_ROOT}/scripts/init.sh apply \
...
- the labels: `type:piece`, `type:sprint`, `type:retro`, the decider label,
  `decision-needed`, `blocked`, `review:passed`, one `track:*` per track and
  one `owner:*` per owner;
...
bash ${PLUGIN_ROOT}/scripts/squad.sh board
```
