...
```bash
bash ${PLUGIN_ROOT}/scripts/squad.sh issue "Sprint N (DD to DD Mon YYYY)" \
  --track <track> --owner <owner> --label type:sprint --body-file sprint.md
bash ${PLUGIN_ROOT}/scripts/squad.sh sprint <piece issue> "Sprint N"
bash ${PLUGIN_ROOT}/scripts/squad.sh move <piece issue> "This sprint"
bash ${PLUGIN_ROOT}/scripts/squad.sh needed-by <decider issue> YYYY-MM-DD
```
