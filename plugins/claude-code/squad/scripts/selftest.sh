...
cat > "$PROJECT/.claude/squad.local.md" <<'SETTINGS'
---
...
chief_of_staff_model: claude-opus-5[1m]
implementer_model: claude-sonnet-5[1m]
reviewer_model: claude-sonnet-5[1m]
clerk_model: claude-haiku-5[1m]
...
SETTINGS
export SQUAD_SETTINGS="$PROJECT/.claude/squad.local.md"
...
cp "$SQUAD_SETTINGS" "$REPOA/.claude/squad.local.md"
...
out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/gitw.sh" save "msg" 2>&1)"
check_contains "gitw: save without paths is refused" "$out" "Scoped paths are required"

out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/gitw.sh" save "msg" -- file.txt 2>&1)"
check_contains "gitw: save on main is refused" "$out" "Refusing to commit directly to main"

git -C "$REPOA" checkout -q -b piece-1-x
out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/gitw.sh" save-all "msg" 2>&1)"
check_contains "gitw: save-all with nothing to commit is refused" "$out" "Nothing to commit"

out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/gitw.sh" submit "t" abc 2>&1)"
check_contains "gitw: a non-numeric issue is refused" "$out" "must be digits only"

out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/gitw.sh" status 2>&1)"
...
out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/gitw.sh" doctor 2>&1)"
...
out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" \
...
out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/handover.sh" --dry-run)"
...
USAGE
out="$(SQUAD_USAGE_FILE="$WORK/usage.json" "$SCRIPTS/claude-quota" --json 2>&1)"
...
out="$(SQUAD_USAGE_FILE="$WORK/usage.json" SQUAD_QUOTA_FORCE=suspend "$SCRIPTS/claude-quota" 2>&1)"; rc=$?
check_contains "quota: the verdict can be forced"    "$out" "suspend:"
check_equal    "quota: a forced suspend exits 1"     "$rc" "1"

out="$(SQUAD_USAGE_FILE="$WORK/nothing.json" "$SCRIPTS/claude-quota" 2>&1)"; rc=$?
check_contains "quota: a missing reading says so"    "$out" "missing:"
check_equal    "quota: and exits 3"                  "$rc" "3"

# --- 7. the hooks -----------------------------------------------------------

out="$(cd "$WORK" && SQUAD_SETTINGS="" CLAUDE_PROJECT_DIR="$WORK/none" bash "$ROOT/hooks/scripts/session-start.sh" 2>&1)"
check_contains "session start: a quota line is always printed" "$out" "quota: "
check_contains "session start: no settings is said plainly"    "$out" "Run /squad:init"
...
out="$(cd "$REPOA" && SQUAD_SETTINGS="$REPOA/.claude/squad.local.md" bash "$SCRIPTS/squad.sh" settings 2>&1)"
check_contains "conductor: the tab is named after the Chief of Staff" "$out" "tab a-person"

out="$(conductor env SQUAD_CONDUCTOR_QUOTA=run FAKE_ZELLIJ_TABS="a-person" \
...
if date -u -d '@0' '+%Y' >/dev/null 2>&1; then
  # A start that left no hand-over behind must not be repeated every ten minutes.
...
# --- 9. the two manifests agree on a version ---------------------------------

plugin_version="$(jq -r '.version // empty' "$ROOT/.claude-plugin/plugin.json" 2>/dev/null)"
marketplace_plugin_version="$(jq -r '.plugins[0].version // empty' "$ROOT/../../../.claude-plugin/marketplace.json" 2>/dev/null)"
marketplace_version="$(jq -r '.version // empty' "$ROOT/../../../.claude-plugin/marketplace.json" 2>/dev/null)"
if [ -z "$plugin_version" ] || [ -z "$marketplace_plugin_version" ]; then
  bad "manifests: both carry a version" "plugin.json='$plugin_version' marketplace.json plugin='$marketplace_plugin_version'"
else
  check_equal "manifests: plugin.json and marketplace.json's plugin entry agree" \
    "$marketplace_plugin_version" "$plugin_version"
fi
check_equal "manifests: marketplace.json's own version matches the plugin entry" \
  "$marketplace_version" "$marketplace_plugin_version"
