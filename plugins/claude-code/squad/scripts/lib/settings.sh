...
#
# The settings live in .claude/squad.local.md as YAML front matter. Nothing in
# this plugin knows a project name, a person, a model or a repository: it all
# comes from that one file. /squad:init writes it by asking the questions once.
...
squad_find_settings() {
  ...
  for dir in "${SQUAD_PROJECT_DIR:-}" "${CLAUDE_PROJECT_DIR:-}"; do
    [ -n "$dir" ] || continue
    [ -r "$dir/.claude/squad.local.md" ] || continue
    printf '%s\n' "$dir/.claude/squad.local.md"
    return 0
  done
  dir="$PWD"
  while [ "$dir" != "/" ] && [ -n "$dir" ]; do
    if [ -r "$dir/.claude/squad.local.md" ]; then
      printf '%s\n' "$dir/.claude/squad.local.md"
      return 0
    fi
    dir="$(dirname "$dir")"
...
squad_load_settings() {
  ...
  file="$(squad_find_settings)" || squad_die \
    "No Squad settings found. Expected .claude/squad.local.md in the project, or SQUAD_SETTINGS pointing at one. Run /squad:init to write it."
...
Done}"
  ...
  : "${SQUAD_QUOTA_COMMAND:=claude-quota}"
  : "${SQUAD_USAGE_COMMAND:=claude-usage}"
  ...
  : "${SQUAD_ARCHITECTURE_REVIEWER_MODEL:=fable}"
  : "${SQUAD_ENGINEERING_REVIEWER_MODEL:=opus}"
  ...
  : "${SQUAD_TRANSCRIPTS_DIR:=$HOME/.claude/projects}"
  : "${SQUAD_MEMORY_STORE:=}"
  : "${SQUAD_HARNESS_CHANGELOG_URL:=https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md}"
  ...
  : "${SQUAD_CONDUCTOR_COMMAND:=claude}"
