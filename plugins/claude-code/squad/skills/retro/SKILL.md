...
`bash ${CLAUDE_PLUGIN_ROOT}/scripts/meeting.sh start retro`
...
Held on the last day of the sprint, before the clock turns over. A conversation
...
bash ${CLAUDE_PLUGIN_ROOT}/scripts/squad.sh board --sprint "Sprint N"
bash ${CLAUDE_PLUGIN_ROOT}/scripts/housekeeping.sh --dry-run
bash ${CLAUDE_PLUGIN_ROOT}/scripts/changelog-review.sh
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/housekeeping.sh
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/squad.sh issue "Retrospective: Sprint N" \
