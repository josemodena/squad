# Squad documentation

Start with the tutorial, then use the reference while working. You do not need to
install a recovery service for your first project.

## Getting started

1. [Requirements and permissions](getting-started/requirements.md)
2. [Installation, updates and removal](getting-started/installation.md)
3. [Your first delivery cycle](getting-started/quickstart.md)

## Guides

- [Workflow, roles and continuation](guides/workflow.md)
- [Codex operation and recovery](guides/codex.md)
- [Claude Code operation and recovery](guides/claude-code.md)
- [Migration from older releases](guides/migration.md)
- [Troubleshooting](guides/troubleshooting.md)

## Reference

- [CLI](reference/cli.md)
- [Configuration and quota readings](reference/configuration.md)
- [Runtime commands and data ownership](reference/runtime.md)
- [Known limitations and support boundaries](reference/limitations.md)
- [Feature comparison](../README.md#features-and-support)

## Developing Squad

- [Contributing](../CONTRIBUTING.md)
- [Architecture and repository layout](development/architecture.md)
- [Testing and live acceptance](development/acceptance.md)
- [Releasing and public-repository preparation](development/releasing.md)
- [Roadmap](development/roadmap.md)
- [Licensing](development/licensing.md)
- [Changelog](../CHANGELOG.md)
